...
export type FleetDuelMode = "duel" | "royale";
...
export function createFleetDuelState(playerIds: string[]): FleetDuelState {
  const mode: FleetDuelMode = playerIds.length === 2 ? "duel" : "royale";
...
export function alivePlayers(state: FleetDuelState): string[] {
  return state.playerIds.filter((id) => state.fleets[id]?.alive);
}

export function resolveBets(state: FleetDuelState, eliminated: string[]): Record<string, number> {
...
export function computeFleetDuelScores(state: FleetDuelState): Record<string, number> {
