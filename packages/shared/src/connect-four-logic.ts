...
export type ConnectFourPhase = "instructions" | "playing" | "match_end" | "round_end" | "ended";
...
export function createConnectFourState(playerIds: string[]): ConnectFourState {
...
export function activePlayers(state: ConnectFourState): [string, string] | null {
...
export function currentTurnPlayer(state: ConnectFourState): string | null {
...
export function markForPlayer(state: ConnectFourState, playerId: string): CfCell {
  const pair = activePlayers(state);
  if (!pair) return "x";
  return playerId === pair[0] ? "x" : "o";
}

export function resetBoard(state: ConnectFourState): void {
  state.board = emptyGrid();
  state.currentPlayerIndex = 0;
  state.winnerId = null;
}

export function nextChallenger(state: ConnectFourState): string | null {
...
export function computeCfRoundScores(state: ConnectFourState): Record<string, number> {
